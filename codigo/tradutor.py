...
Original file is located at
    https://colab.research.google.com/drive/1KfC6e7euDXoEnnOktUk4Koqt1DFlFywP
...
gu, pt = create_dataset('pairs.txt', 3)
...
Usaremos a divisão na forma 80-20 onde 80% são para treino e 20% para teste começando """

# Try experimenting with the size of that dataset
num_examples = 5967
input_tensor, target_tensor, inp_lang, targ_lang = load_dataset('pairs.txt', num_examples)
...
Implement an encoder-decoder model with attention which you can read about in the TensorFlow Neural Machine Translation 
(seq2seq) tutorial. 
This example uses a more recent set of APIs. This notebook implements the attention equations from the seq2seq tutorial. The following diagram shows that each input words is assigned a weight by the attention mechanism which is then used by the decoder to predict the next word in the sentence. The below picture and formulas are an example of attention mechanism from Luong's paper.
"""
...
translate('Porque Deus o mundo tanto')
...
def saving_result(dataset, acc, desv):
	...
	file.write('Data set: ' + dataset + '\tAccuracy: ' + str(acc) + 'Desvio Padrão: ' + str(desv) + '\n')
